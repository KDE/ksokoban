...
void Map::map(int x, int y, int val)
{
    ...
    if ((map(x, y) & (OBJECT | GOAL)) == OBJECT)
        objectsLeft_--;
    if ((val & (OBJECT | GOAL)) == OBJECT)
        objectsLeft_++;
...
void Map::setMap(int x, int y, int bits)
{
    assert((map(x, y) & bits) == 0);
    if (goal(x, y) && ((bits & OBJECT) == OBJECT))
        objectsLeft_--;
...
void Map::clearMap(int x, int y, int bits)
{
...
bool Map::fillFloor(int x, int y)
{
    if (badCoords(x, y))
        return false;
    if ((currentMap_[y + 1][x + 1] & (WALL | FLOOR)) != 0)
        return true;
...
bool Map::step(int _x, int _y)
{
    ...
    if (_y > ypos_)
        yd = 1;
    if (badDelta(xd, yd) || badCoords(_x, _y))
        return false;
...
bool Map::push(int _x, int _y)
{
    ...
    if (_y > ypos_)
        yd = 1;
    if (badDelta(xd, yd) || badCoords(_x + xd, _y + yd))
        return false;

    int x = xpos_ + xd, y = ypos_ + yd;
    if (!object(x, y))
        return false;
    if (!empty(_x + xd, _y + yd))
        return false;
...
bool Map::unstep(int _x, int _y)
{
    return Map::step(_x, _y);
}

bool Map::unpush(int _x, int _y)
{
    assert(!badCoords(xpos_, ypos_));
    assert(empty(xpos_, ypos_));
